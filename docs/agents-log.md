...
This document consolidates release history and high-impact architectural decisions for Agent Commander.
...
- Added runtime config knobs: `telegram_streaming_enabled` and `telegram_streaming_min_update_ms`.
...
### AD-007: JSONL-only state path (no SQLite cleanup path)
